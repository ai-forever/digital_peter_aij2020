...
Еще раз подчеркнем, что выбрав определенные значения `x` и `y` можно восстановить последовательность строк в определенном документе - это будут номера `z` по возрастанию для фиксированных `x`, `y`.
